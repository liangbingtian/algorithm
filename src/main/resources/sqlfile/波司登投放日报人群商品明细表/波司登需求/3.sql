...
case
when dwd_jd_jzt_kc_sku_report.date_str>='${weekStartDay}' AND dwd_jd_jzt_kc_sku_report.date_str<='${endDay}' then '本周数据'
when dwd_jd_jzt_kc_sku_report.date_str>=DATE_FORMAT(DATE_SUB(STR_TO_DATE('${weekStartDay}', '%Y-%m-%d'), INTERVAL 1 WEEK), '%Y-%m-%d') AND dwd_jd_jzt_kc_sku_report.date_str<=DATE_FORMAT(DATE_SUB(STR_TO_DATE('${endDay}', '%Y-%m-%d'), INTERVAL 1 WEEK), '%Y-%m-%d') then '上周数据'
when dwd_jd_jzt_kc_sku_report.date_str>='${monthStartDay}' AND dwd_jd_jzt_kc_sku_report.date_str<='${endDay}' then '本期月数据'
end AS '时间所属'
...
AND dwd_jd_jzt_kc_sku_report.date_str>=LEAST('2024-02-01','2024-01-28')
AND dwd_jd_jzt_kc_sku_report.date_str<='2024-02-04'
GROUP BY
case
when business in ('智能投放','京速推','智能投放（原京速推）') and campaign_name IN ('全店推广计划', '首焦海投计划', '经典海投计划', 'PUSH海投计划', '海投新品计划', '测款计划', '频道活动计划', '预售推广计划', '直降促销计划') then '海投'
when business in ('智能投放','京速推','智能投放（原京速推）') and campaign_name not IN ('全店推广计划', '首焦海投计划', '经典海投计划', 'PUSH海投计划', '海投新品计划', '测款计划', '频道活动计划', '预售推广计划', '直降促销计划') then '京速推'
