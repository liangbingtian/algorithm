...
sum(clicks) AS '点击量',
basic_jd_udf_adserving_longines_crowd_set.type as '人群类别',
...
case
when date_str>='2024-02-01' and date_str<='2024-02-28' then '本周数据'
when date_str>=DATE_FORMAT(DATE_SUB(STR_TO_DATE('2024-02-01', '%Y-%m-%d'), INTERVAL 1 WEEK), '%Y-%m-%d') and date_str<=DATE_FORMAT(DATE_SUB(STR_TO_DATE('2024-02-28', '%Y-%m-%d'), INTERVAL 1 WEEK), '%Y-%m-%d') then '上周数据'
when date_str>=DATE_FORMAT(DATE_SUB(STR_TO_DATE('2024-02-01', '%Y-%m-%d'), INTERVAL 1 YEAR), '%Y-%m-%d') and date_str<=DATE_FORMAT(DATE_SUB(STR_TO_DATE('2024-02-28', '%Y-%m-%d'), INTERVAL 1 YEAR), '%Y-%m-%d') then '去年数据'
end as '时间所属'
...
and gift_flag=0
and order_status=1
AND date_str>=DATE_FORMAT(DATE_SUB(STR_TO_DATE('2024-02-01', '%Y-%m-%d'), INTERVAL 1 YEAR), '%Y-%m-%d')
AND date_str<='2024-02-28'
GROUP BY
case when dwd_jd_jzt_crowd_report.username='Longines-2021' then '自营'
when dwd_jd_jzt_crowd_report.username='Longines_POP2023'   then '官旗'
end,
...
date_str,
basic_jd_udf_adserving_longines_crowd_set.type