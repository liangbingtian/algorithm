...
WHERE dwd_jd_jzt_keyword_report.username in ('MD2024-投放', 'MDtoufang2024')
  ...
  and gift_flag = 0
  and order_status = 1
  AND dwd_jd_jzt_keyword_report.date_str >= DATE_FORMAT(STR_TO_DATE('${startDay}', '%Y-%m-%d'), '%Y%m%d')
  AND dwd_jd_jzt_keyword_report.date_str <= DATE_FORMAT(STR_TO_DATE('${endDay}', '%Y-%m-%d'), '%Y%m%d')
GROUP BY
    case
        when ifnull(kt1.keyword_type, kt.keyword_type) is null then '无词类'
        when ifnull(kt1.keyword_type, kt.keyword_type) = 1 then '类目词'
