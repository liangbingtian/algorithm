(SELECT a.keyword as '关键词', ifnull(skuCid3, '其他三级类目id') as '三级类目id', ifnull(max(skuCName3), '其他三级类目') as '三级类目名称'
...
union all
(SELECT  a.keyword as '关键词', ifnull(skuCid3, '其他三级类目id') as '三级类目id', ifnull(max(skuCName3), '其他三级类目') as '三级类目名称', ifnull(skuCid2, '其他二级类目id') as '二级类目id',ifnull(max(c.sku_name), '无SKU名称') as 'sku名称',
         case
             when ifnull(max(skuCname2), '其他二级类目') = '洗发护发' then '洗护'
             when ifnull(max(skuCname2), '其他二级类目') = '美发造型' then '造型'
             else ifnull(max(skuCname2), '其他二级类目')
         ...
         , SUM(all_cost) AS '花费', SUM(tot_ord_sum) AS 'GMV', sum( tot_cart_cnt ) AS '加购数', sum(tot_ord_cnt) as '总订单行', SUM(impressions) AS '展现量', SUM(clicks) AS '点击量', sum(dir_ord_sum) as '直接成交金额',  case
            when date_str >= DATE_FORMAT(STR_TO_DATE('2024-11-18', '%Y-%m-%d'), '%Y%m%d') and date_str <= DATE_FORMAT(STR_TO_DATE('2024-11-24', '%Y-%m-%d'), '%Y%m%d') then '本周数据'
            ...
            when date_str >= DATE_FORMAT(DATE_SUB(STR_TO_DATE('2024-11-18', '%Y-%m-%d'), INTERVAL 1 YEAR), '%Y%m01') and
                 date_str <= DATE_FORMAT(DATE_SUB(STR_TO_DATE('2024-11-18', '%Y-%m-%d'), INTERVAL 1 YEAR), '%Y%m31') then '去年本周数据'
 ...
 WHERE a.username in ('施华蔻专业投放')
   ...
   and gift_flag = 0
   and order_status = 1
   AND (date_str between DATE_FORMAT(STR_TO_DATE('2024-11-18', '%Y-%m-%d'), '%Y%m%d') and DATE_FORMAT(STR_TO_DATE('2024-11-24', '%Y-%m-%d'), '%Y%m%d')
     or
     ...
     or
        date_str between DATE_FORMAT(DATE_SUB(STR_TO_DATE('2024-11-18', '%Y-%m-%d'), INTERVAL 1 YEAR), '%Y%m01') and DATE_FORMAT(DATE_SUB(STR_TO_DATE('2024-11-18', '%Y-%m-%d'), INTERVAL 1 YEAR), '%Y%m31')
     or
