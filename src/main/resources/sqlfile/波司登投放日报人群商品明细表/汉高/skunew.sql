SELECT ifnull(skuCid3, '其他三级类目id') as '三级类目id', ifnull(ifnull(max(c.cid3_name), max(skuCName3)), '其他三级类目') as '三级类目名称', ifnull(skuCid2, '其他二级类目id') as '二级类目id',
    ...
    when date_str >= DATE_FORMAT(DATE_SUB(STR_TO_DATE('${startDay}', '%Y-%m-%d'), INTERVAL 1 YEAR), '%Y-%m-01') and
         date_str <= DATE_FORMAT(DATE_SUB(STR_TO_DATE('${startDay}', '%Y-%m-%d'), INTERVAL 1 YEAR), '%Y-%m-31') then '去年本周数据'
...
WHERE a.username in ('施华蔻专业投放')
  ...
  AND (date_str between '${startDay}' and '${endDay}'
    ...
    or
       date_str between DATE_FORMAT(DATE_SUB(STR_TO_DATE('${startDay}', '%Y-%m-%d'), INTERVAL 1 YEAR), '%Y-%m-01') and DATE_FORMAT(DATE_SUB(STR_TO_DATE('${startDay}', '%Y-%m-%d'), INTERVAL 1 YEAR), '%Y-%m-31')
    or
